...
def dsSales(nreg=500):
    ...
    for i in np.arange(1,nreg+1):
        ...
        catproduto.append(''.join(np.random.choice(alpha, 2)))
        vendedores.append(''.join(np.random.choice(alpha, 5)))
        ...
        lojas.append(''.join(np.random.choice(alpha, 4)))
        clientes.append(''.join(np.random.choice(alpha, 6)) + ' ' + ''.join(np.random.choice(alpha, 8)))
        sexo.append(''.join(np.random.choice(['m', 'f', 'a', '-'], 1)))
